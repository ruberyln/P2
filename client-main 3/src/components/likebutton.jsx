...
function LikeButton({ likes, disLikes, fetchPosts, postId }) {
    ...
    const clearUpVote = async (e) => {
        e.preventDefault();

        await commonApi({
            action: "clearUpVoting",
            data: {
                userId: user._id,
                postId: postId
            },
    ...
    const clearDownVote = async (e) => {
        e.preventDefault();

        await commonApi({
            action: "clearDownVoting",
            data: {
                userId: user._id,
                postId: postId
            },
    ...
    return (
        <>{likeactive && <Button onClick={clearUpVote} color="secondary">
            <FavoriteIcon style={{ color: "secondary" }} />
        </Button>}
            {!likeactive && <Button onClick={handleLikes} color="secondary">< FavoriteBorderIcon />
            </Button>}{like}

            {dislikeactive && <Button onClick={clearDownVote} color="secondary">< ThumbDownIcon />
            </Button>}
